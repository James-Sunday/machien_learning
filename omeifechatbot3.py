...
updated_key = "af99942dd22c4ab38c24935a972e5ced"
...
class chatbot():
    ...
    def NLU(self,role,text,name, emotion, age, gender):
        url = 'http://10.10.40.2:10204/query'
        data = {'role':role,'text':text,'name':name, 'emotion':emotion, 'age':age, 'gender':gender}
        ...
        #check if it's saying thanks
         ...
         ## conversation
            ...
            if ai.text=="ERROR":
                ...
                # wave_hand.jaw()
                # wave_hand.all_actions()
                # wave_hand.all_actions2()
            ...
            elif "Look at me" in ai.text:
                look_at=arduino
                look_at.look_at_me()
                print("looking at you")
                res="i'm looking at you now"
            ...
            #     look_at.thinker()
                ...
                speak.all_movement()
                speak.default_mode()
                res = ai.NLU(role,ai.text,updated_name, updated_emotion, updated_age, updated_gender)['text']
                word = res.split()
                words=len(word)
                print(words)
...
def all_action():
    ...
    for _ in range(5):
        all_move.jaw()
        all_move.default_mode()
    for _ in range(2):
        all_move.all_actions()
        all_move.default_mode()
        # all_move.all_movement()
        # all_move.default_mode()
        all_move.all_actions()
